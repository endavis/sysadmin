...
class AppClass:
    ...
    def build_totals_sheet(self):
        ...
        self.totals_ws.write(
            "B1",  # pyright: ignore[reportArgumentType]
            "Total Provisioned (Licensing) (TiB)",  # pyright: ignore[reportArgumentType]
        )
        ...
        self.totals_ws.write(
            "D1", "Percent Used"  # pyright: ignore[reportArgumentType]
        )
        self.totals_ws.set_column(3, 3, 13)

        self.totals_ws.write(
            "A2", "Azure CVO RW"  # pyright: ignore[reportArgumentType]
        )
        self.totals_ws.write_formula(
            "B2",
            formula=(
        ...
        self.totals_ws.write(
            "A3", "Azure CVO DP"  # pyright: ignore[reportArgumentType]
        )
        self.totals_ws.write_formula(
            "B3",
            formula=(
        ...
        self.totals_ws.write(
            "A4", "Azure CVO HA RW"  # pyright: ignore[reportArgumentType]
        )
        self.totals_ws.write_formula(
            "B4",
            formula=(
        ...
        self.totals_ws.write(
            "A5", "Azure CVO HA DP"  # pyright: ignore[reportArgumentType]
        )
        self.totals_ws.write_formula(
            "B5",
            formula=(
        ...
        self.totals_ws.write(
            "A6", "Azure Totals"  # pyright: ignore[reportArgumentType]
        )
        self.totals_ws.write_formula(
        ...
        self.totals_ws.write(
            "A10", "AWS CVO HA RW"  # pyright: ignore[reportArgumentType]
        )
        self.totals_ws.write_formula(
            "B10",
            formula=(
        ...
        self.totals_ws.write(
            "A11", "AWS CVO HA DP"  # pyright: ignore[reportArgumentType]
        )
        self.totals_ws.write_formula(
            "B11",
            formula=(
    ...
    def build_volume_sheet(self):
        ...
        #     "Region", "Cluster Type", "Data Type", "Volume", "State", "Tags",
        #     "Provisioned Size (Licensing) [TiB]", "Used Size TiB", "%% used"
        # ]
        provision_col_header = "Provisioned (Licensing) TiB"
        used_col_header = "Used TiB"
        percentused_header = "%% Used"
        percentused_formula = (
            f"=[@[{used_col_header}]]/[@[{provision_col_header}]] * 100"
        )
    ...
    def build_usage_sheet(self):
                    ...
                    for env in self.divisions[div][bu][app]:
                        for subapp in self.divisions[div][bu][app][env]:
                            for cloud in self.divisions[div][bu][app][env][subapp]:
                                for region in self.divisions[div][bu][app][env][subapp][
                                    cloud
                                ]:
                                    for cluster_type in self.divisions[div][bu][app][
                                        env
                                    ][subapp][cloud][region]:
                                        for data_type in self.divisions[div][bu][app][
                                            env
                                        ][subapp][cloud][region][cluster_type]:
                                            if self.divisions[div][bu][app][env][
                                                subapp
                                            ][cloud][region][cluster_type][data_type]:
                                                func_filter = (
        ...
        for row_num, row_data in enumerate(usage_data, start=1):
            for col_num, cell in enumerate(row_data):
                ...
                cell_length = len(str(cell))
                if (not str(cell).startswith("=")) and cell_length > usage_col_widths[
                    col_num
                ]:
        ...
        try:
            with HostConnection(
                ...
                for volume in volumes:
                    size = float(
                        f"{approximate_size_specific(volume['size'], 'TiB',withsuffix=False):.7f}"
                    )
                    if volume["state"] == "offline":
                        used = 0
                    else:
                        used = float(
                            f"{approximate_size_specific(volume['space']['used'],'TiB', withsuffix=False):.7f}"
                        )
                    ...
                    self.app_instance.volume_data.append(
                        [
                            self.div,  # type: ignore
                            self.bu,  # type: ignore
                            self.name,
            ...
            logging.error(
                f"Could not retrieve data for {self.name} {volume['name']} {e}",
                exc_info=e,
            )
