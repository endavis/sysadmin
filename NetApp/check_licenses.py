...
class AppClass:
    ...
    def go(self):
        ...
        mailfrom = (
            self.config.settings["settings"]["licensing"]["mailfrom"] or "Unknown Email"
        )
        mailto = (
            self.config.settings["settings"]["licensing"]["mailto"] or "Unknown Email"
        )
        ...
        if self.license_issues:
            ...
            for item in self.license_issues:
                ...
                license_type = (
                    item["license type"] if "license type" in item else "Unknown"
                )
                days = item["days checked"] if "days checked" in item else "Unknown"
                if isinstance(days, (int, float)) and days < 0:
                    email_body.append(
                        f"    <p class='red-white'>{item['cluster']} - {owner}"
                        f" - {serial} - {license_type} has expired {abs(days)} days"
                        f" ago on {item['expires']}!</p>"
                    )
                else:
                    email_body.append(
                        f"    <p class='yellow-black'>{item['cluster']} - {owner}"
                        f" - {serial} - {license_type} expires in {days}"
                        f" days on {item['expires']}.</p>"
                    )
...
class ClusterData:
    ...
    def process_data(self):
        ...
        if not self.fetched_data["licenses"]:
            self.app_instance.license_issues.append(
                {
                    "cluster": self.name,
                    "owner": "",
        ...
        for license in self.fetched_data["licenses"]:
            for item in license["licenses"]:
                ...
                if item["owner"] in self.fetched_data["nodes"]:
                    ...
                    self.fetched_data["nodes"][item["owner"]][
                        "license_type"
                    ] = license_type
                ...
                if "expiry_time" in item and item["owner"] != "none":
                    ...
                    if days_difference < days_to_check:
                        ...
                        if days_difference < 0:
                            logging.info(
                                f"{self.name} - {item['owner']} - "
                                f"{item['serial_number']} - {license_type} has expired"
                                f" {abs(days_difference)} days ago on {expiry_time}!"
                            )
                        else:
                            logging.info(
                                f"{self.name} - {item['owner']} - "
                                f"{item['serial_number']} - {license_type} expires in"
                                f" {days_difference} days on {expiry_time}"
                            )

        for node in self.fetched_data["nodes"]:

            if (
                "has_license" not in self.fetched_data["nodes"][node]
                or not self.fetched_data["nodes"][node]["has_license"]
            ):
                ...
                self.app_instance.license_issues.append(
                    {
                        ...
                        "days checked": -1,
                        "serial number": (
                            self.fetched_data["nodes"][node]["serial_number"]
                            if "serial_number" in self.fetched_data["nodes"][node]
                            else "Unknown"
                        ),
                        "expires": -1,
                        "license type": "None",
...
if __name__ == "__main__":
    ...
    args = argp(
        script_name=script_name, description="check clusters for licensing issues"
    )
    config = Config(
