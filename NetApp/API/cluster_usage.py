...
class ClusterData:
    ...
    def gather_data(self):
        with HostConnection(self.ip, username=config.username, password=config.enc, verify=False):
            cluster = Cluster()
            ...
            for volume in volumes:
                ...
                size_set_as_80 = 0
                savings = 0
                if volume['space']['percent_used'] < 80:
                    size_set_as_80 = volume['space']['used'] * 1.2
                    savings = volume['size'] - size_set_as_80
                    if size_set_as_80 < 1073741824: # minimum size should be 1 GiB
                        size_set_as_80 = 1073741824
                else:
...
if __name__ == '__main__':
    ...
    items = config.search(args.filters)
