...
class AppClass:
    ...
    def format_azure_info(self, azure_info):
        ...
        self.format_table_row_text(
            "Azure Subscription ID",
            self.config.data["azure"][azure_info["location"]]["id"],
        )
        ...
        self.format_table_row_link(
            "Azure Resource Group", resource_group_id, resource_group_url
        )
            ...
            for app in apps_list:
                ...
                with self.tag("li"):
                    with self.tag("details", ("class", det_class)):
                        ...
                        with self.tag("ul"):
                            self.format_environments(
                                new_where, apps[app], new_search_terms
                            )
        ...
        for environment in environments_list:
            ...
            with self.tag("li"):
                with self.tag("details", ("class", det_class)):
                    ...
                    with self.tag("ul"):
                        self.format_subapps(
                            new_where, environments[environment], new_search_terms
                        )
...
class ClusterData:
    ...
    def build_cloud_info(self):
        if hasattr(self, "azure"):
            ...
            self.azure["resource_group_id"] = build_azure_id(
                sub_id, self.azure["resource_group"]
            )
            self.azure["resource_group_url"] = build_azure_portal_link(
                self.azure["resource_group_id"]
            )
    ...
    def format(self, tag, text):
        ...
        with self.html_tag("li"):
            with self.html_tag(
                "details",
                ("class", f"{self.ele_class}-active" if self.ele_class else ""),
            ):
                    ...
                    with self.html_tag("li"):
                        with self.html_tag("table", ("class", "custom-table")):
                            self.app_instance.format_table_row_text(
                                "Version",
                                self.fetched_data["cluster"]["version"]["full"],
                            )
                            self.app_instance.format_table_row_text(
                                "Cluster Management IP",
                                self.fetched_data["cluster"]["management_interfaces"][
                                    0
                                ]["ip"]["address"],
                            )
                                ...
                                with self.html_tag("li"):
                                    with self.html_tag(
                                        ...
                                        self.app_instance.format_table_row_text(
                                            "Domains",
                                            ", ".join(
                                                self.fetched_data["cluster"][
                                                    "dns_domains"
                                                ]
                                            ),
                                        )
                                        for i, name_server in enumerate(
                                            self.fetched_data["cluster"]["name_servers"]
                                        ):
                                            self.app_instance.format_table_row_text(
                    ...
                    with self.html_tag("li"):
                        with self.html_tag("table", ("class", "custom-table")):
                            ...
                            for svm_interface in svm_data["ip_interfaces"]:
                                ip_interface = self.fetched_data["interfaces"][
                                    svm_interface["name"]
                                ]
                                self.app_instance.format_table_row_text(
                                    ip_interface["name"],
            ...
            with self.html_tag("details"):
                ...
                with self.html_tag("ul"):
                    with self.html_tag("li"):
                        with self.html_tag("table", ("class", "custom-table")):
                            self.app_instance.format_table_row_text(
                                "Domains", ", ".join(svm_data["dns"]["domains"])
                            )
            ...
            with self.html_tag("details"):
                ...
                with self.html_tag("ul"):
                    with self.html_tag("li"):
                        with self.html_tag("table", ("class", "custom-table")):
                            self.app_instance.format_table_row_text(
                                "Enabled", cifs_data["enabled"]
                            )
                            self.app_instance.format_table_row_text(
                                "Name", cifs_data["name"]
                            )
                            self.app_instance.format_table_row_text(
                                "Domain", cifs_data["ad_domain"]["fqdn"]
                            )
                            self.app_instance.format_table_row_text(
                                "Domain", cifs_data["ad_domain"]["organizational_unit"]
                            )
                    with self.html_tag("li"):
                        with self.html_tag("details"):
                            with self.html_tag("summary"):
                                ...
                                with self.html_tag("li"):
                                    with self.html_tag(
                                        ...
                                        for header, key in items:
                                            if isinstance(
                                                cifs_data["security"][key], list
                                            ):
                                                self.app_instance.format_table_row_text(
                                                    header,
                                                    ", ".join(
                                                        cifs_data["security"][key]
                                                    ),
                                                )
                                            else:
                                                self.app_instance.format_table_row_text(
                                                    header, cifs_data["security"][key]
                                                )
    ...
    def format_netapp_node(self, node_data):
        management_link = (
            f"https://{node_data['management_interfaces'][0]['ip']['address']}"
        )
        vm_id = None
        vm_url = None
        match self.cloud:
                ...
                with self.html_tag("ul"):
                    with self.html_tag("li"):
                        with self.html_tag("table", ("class", "custom-table")):
                            self.app_instance.format_table_row_text(
                                "Node Management IP",
                                node_data["management_interfaces"][0]["ip"]["address"],
                            )
                            self.app_instance.format_table_row_text(
                                "Serial Number", node_data["serial_number"]
                            )
                            self.app_instance.format_table_row_link(
                                "System Manager", "Link", management_link
                            )
                            ...
                            if "vm_name" != "Unknown" and vm_id and vm_url:
                                self.app_instance.format_table_row_text(
                                    f"{vm_type} Name", vm_name
                                )
                                self.app_instance.format_table_row_link(
                                    vm_type, vm_id, vm_url
                                )
...
if __name__ == "__main__":
    args = argp(
        ...
        description="build html page of endpoints and mostly static information",
    )
