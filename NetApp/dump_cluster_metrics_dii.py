...
class AppClass:
    def __init__(self, name, clusters, config: Config):
        ...
        self.config = config
        tdate: datetime.datetime = datetime.datetime.strptime(
            self.config.args.date,  # pyright: ignore[reportOptionalMemberAccess]
            "%Y-%m-%d",
        )
        ...
        self.clusterdata = {}
        self.dii_api_client = (
            libs.dii.api.DIIAPIClient(  # pyright: ignore[reportAttributeAccessIssue]
                self.config
            )
        )
...
class ClusterData:
    ...
    def gather_data_for_volume(self, volume_name: str, vserver_name: str):
        ...
        try:
            ...
            filter_expr = (
                f'vserver_name = "{vserver_name}" AND volume_name = "{volume_name}"'
            )
            ...
            for metric in results:
                for data in results[metric][0]["timeseries"]:
                    ...
                    if timestamp not in current_metrics:
                        if not first:
                            logging.info(
                                f"Got a new timestamp for metric {metric}"
                                f' and time {data["time"]}'
                            )
                    ...
                    try:
                        ...
                        logging.debug(
                            f"bad timestamp data: {volume_name} {metric} {data}"
                        )  # , exc_info=e)
            ...
            for item in bad_keys:
                if (
                    item in current_metrics
                    and "timestamp" in current_metrics[item]
                    and len(current_metrics[item]) == 1
                ):
            ...
            for item in current_metrics:
                if len(current_metrics[item]) != number_of_metrics:
                    logging.error(
                        f"timestamp {item} does not have {number_of_metrics} keys"
                    )
            ...
            logging.error(
                f"Could not retrieve data for {self.name}:{vserver_name}:{volume_name}",
                exc_info=e,
            )
...
if __name__ == "__main__":
    ...
    args = argp(
        ...
        description=(
            "dump volume metrics for a specific day (+/- done day for 3 days total)"
        ),
        parse=False,
    )
    args.parser.add_argument(
        ...
        "--date",
        type=str,
        help="the date of the form YYYY-MM-DD, example: 2025-04-13",
        default="",
        required=True,
    )
    args.parse()
    ...
    # metrics = ["read_ops", "write_ops", "read_throughput",
    ...
    #     metrics=metrics,
    #     # Note : boolean operators must be CAPITAL LETTERS
    #     filter_expr=()'cluster_name = "ZUSE2PDAXCST210" AND vserver_name'
    #                 ' = "svm_ZUSE2PDAXCST210" AND volume_name = "PD2TY2024_E2210"'),
